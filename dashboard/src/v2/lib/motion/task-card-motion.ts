...
export function useTaskCardMotion(ref: RefObject<HTMLElement>, status: string, isReducedMotion: boolean, index: number = 0) {
  ...
  useLayoutEffect(() => {
    ...
    gsap.fromTo(ref.current,
      { opacity: 0, y: 15 },
      { opacity: 1, y: 0, duration: 0.4, ease: "power2.out", delay: index * 0.05, clearProps: "transform,opacity", overwrite: "auto" }
    );
  ...
  useEffect(() => {
    ...
    gsap.fromTo(ref.current,
      ...
      { scale: 1, boxShadow: 'none', duration: 0.3, ease: "power2.out", clearProps: "boxShadow,transform", overwrite: "auto" }
    );
  ...
  useEffect(() => {
    ...
    if (isDragging) {
      ...
      gsap.to(ref.current, {
        ...
        duration: parseFloat(MOTION_TOKENS.timing.fast) / 1000,
      ...
      gsap.to(ref.current, {
        ...
        opacity: 1,
        boxShadow: 'none',
        duration: parseFloat(MOTION_TOKENS.timing.standard) / 1000,
