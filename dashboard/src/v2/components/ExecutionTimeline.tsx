...
export const ExecutionTimeline: FunctionComponent<ExecutionTimelineProps> = ({ activeSprintRuns }) => {
  ...
  return (
    <div>
      <span className="text-[8px] font-bold uppercase tracking-[0.15em] text-slate-500 block mb-3">Runtime Timeline</span>
      {timelineEvents.length === 0 ? (
        <p className="text-[11px] text-slate-500 dark:text-slate-600 font-mono">No task run events recorded yet.</p>
      ) : (
