...
export const IconButton: FunctionComponent<IconButtonProps> = memo(({ children, className = "", title, "aria-label": ariaLabel, pending = false, disabled, onClick, ...props }) => {
    ...
    const button = (
        ...
        >
            <div style={{ transitionDuration: tokens.controlFeedback.duration, transitionTimingFunction: tokens.controlFeedback.ease }} className={`flex items-center justify-center transition-opacity ${childrenOpacity}`}>
                {children}
            </div>

            <div style={{ transitionDuration: tokens.controlFeedback.duration, transitionTimingFunction: tokens.controlFeedback.ease }} className={`absolute inset-0 flex items-center justify-center transition-opacity ${isPending ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                <Loader2 className={`w-5 h-5 ${reducedMotion ? "" : "animate-spin"}`} aria-hidden="true" />
            </div>

            <div style={{ transitionDuration: tokens.controlFeedback.duration, transitionTimingFunction: tokens.controlFeedback.ease }} className={`absolute inset-0 flex items-center justify-center transition-opacity ${isSuccess ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                <Check className="w-5 h-5 text-status-green" strokeWidth={3} aria-hidden="true" />
            </div>

            <div style={{ transitionDuration: tokens.controlFeedback.duration, transitionTimingFunction: tokens.controlFeedback.ease }} className={`absolute inset-0 flex items-center justify-center transition-opacity ${isError ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
