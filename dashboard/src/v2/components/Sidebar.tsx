    ...
    return (
        ...
        >
            ...
            <div ref={navRef} className="flex-1 px-4 flex flex-col relative z-10">
                <div className="px-4 text-[9px] font-bold text-slate-500 dark:text-slate-600 uppercase tracking-[0.2em] mb-3">Workspace</div>
                {navItems.map((item, idx) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <item.icon aria-hidden="true" className={`relative z-10 w-4 h-4 transition-all duration-300 ${isActive ? 'text-signal-600 dark:text-signal-400 drop-shadow-[0_0_8px_rgba(0,224,160,0.5)]' : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300'}`} strokeWidth={isActive ? 2 : 1.5} />

                            <span className={`relative z-10 font-medium text-sm tracking-wide transition-colors duration-300 ${isActive ? 'text-slate-900 dark:text-white font-semibold' : 'text-slate-600 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300'}`}>
                                {item.label}
                            </span>
                        </button>
                    );
            ...
            <div className="px-4 relative z-10">
                <button aria-label="Settings" className="relative w-full flex items-center gap-3.5 px-5 py-3 rounded-2xl transition-colors duration-200 text-left group overflow-hidden focus-visible:ring-2 focus-visible:ring-signal-500/40 focus-visible:rounded-2xl focus-visible:z-10">
                    ...
                    <Settings aria-hidden="true" className="relative z-10 w-4 h-4 text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300 group-hover:rotate-90 transition-all duration-700 ease-in-out" strokeWidth={1.5} />
                    <span className="relative z-10 font-medium text-sm tracking-wide text-slate-600 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors duration-300">Settings</span>
                </button>
            </div>
