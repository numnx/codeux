...
export const AddProjectModal: FunctionComponent<AddProjectModalProps> = ({ onClose, onAdd, initialSourceType }) => {
  ...
  useLayoutEffect(() => {
    const dBackdrop = reducedMotion ? 0 : MODAL_MOTION.backdrop.duration;
    const dCard = reducedMotion ? 0 : MODAL_MOTION.entry.duration;
    gsap.fromTo(backdropRef.current, { opacity: 0 }, { opacity: 1, duration: dBackdrop, ease: MODAL_MOTION.backdrop.ease });
    gsap.fromTo(
      cardRef.current,
      {
  ...
  const handleClose = () => {
    ...
    const dCard = reducedMotion ? 0 : MODAL_MOTION.exit.duration;
    const dBackdrop = reducedMotion ? 0 : MODAL_MOTION.backdrop.duration;
    ...
    gsap.to(backdropRef.current, { opacity: 0, duration: dBackdrop, delay: reducedMotion ? 0 : 0.05, onComplete: onClose });
  };
      ...
      if (sourceType === "new_project") {
        await Promise.resolve(onAdd({
          ...
          ...(newInitMode === "new-remote"
            ? { remoteProvider: newProvider, isPrivate: newIsPrivate }
            : {}),
        }));
      } else {
  ...
  const handleSourceTypeChange = (type: SourceType) => {
    ...
    if (fieldsRef.current) {
      ...
      gsap.fromTo(conditionalFields, { y: 12, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.06, duration: 0.35, ease: "power3.out" });
    }
  };
                            ...
                            >
                              ...
                              <span className="min-w-0 flex-1">
                                <span className="flex items-center gap-1.5 text-xs font-black uppercase tracking-[0.12em]">
                                  ...
                                  {key === "previewScript" && (
                                    <Info
                                      className="h-3.5 w-3.5 cursor-help text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
