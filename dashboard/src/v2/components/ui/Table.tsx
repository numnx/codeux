...
export function TableBody({ children }: { children: ComponentChildren }) {
  return <tbody role="rowgroup" className="block lg:table-row-group">{children}</tbody>;
}
...
export function TableCell({ children, className = "", isFirst, isLast, isHeader, align = "left", colSpan, mobileLabel, ariaSort, id, headers }: TableCellProps) {
  ...
  if (isHeader) {
    ...
    return (
      <th scope="col" aria-sort={ariaSort} id={id}
  ...
  return (
    ...
    >
      {mobileLabel && (
        <span aria-hidden="true" className="mb-1.5 block text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 lg:hidden">
          {mobileLabel}
        </span>
      )}
      {children}
