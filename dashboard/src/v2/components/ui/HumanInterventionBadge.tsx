...
export const HumanInterventionBadge: FunctionComponent<HumanInterventionBadgeProps> = ({
  ...
  const isSystemStop = summary.ownerType === "system" || summary.ownerType === "worker";
  const displayLabel = isSystemStop ? "System stopped" : label;
  ...
  return (
    <div className="group/intervention relative inline-flex" title={formatHumanInterventionTooltip(summary)}>
      ...
      >
        {isSystemStop ? (
          <Bot className={compact ? "h-3.5 w-3.5" : "h-4 w-4"} strokeWidth={2.2} />
        ) : (
      ...
      <div
        className={`pointer-events-none absolute top-[calc(100%+0.75rem)] z-40 w-80 max-w-[80vw] rounded-[1.3rem] border ${
          isSystemStop
            ? "border-slate-400/20 dark:border-slate-400/20"
            : "border-status-amber/20 dark:border-status-amber/20"
        } bg-white/94 p-4 text-left shadow-[0_20px_44px_rgba(15,23,42,0.14)] backdrop-blur-xl opacity-0 transition-all duration-200 group-hover/intervention:translate-y-0 group-hover/intervention:opacity-100 dark:bg-void-800/94 ${
          compact ? "translate-y-1" : "translate-y-2"
        } ${tooltipAlignment}`}
      >
        <div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] ${
          isSystemStop ? "text-slate-500 dark:text-slate-400" : "text-status-amber"
        }`}>
          {isSystemStop ? (
          ...
          {isSystemStop ? "Stopped automatically" : "Human Intervention"}
        </div>
