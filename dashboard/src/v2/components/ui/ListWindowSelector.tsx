  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500 flex items-center gap-2 rounded-xl border border-black/[0.08] bg-white px-3 py-2 text-xs font-semibold tracking-wide text-slate-600 transition-colors hover:border-signal-500/50 hover:bg-signal-500/[0.02] dark:border-white/[0.08] dark:bg-void-900 dark:text-slate-300 dark:hover:border-signal-500/50"
      >
      ...
      {isOpen && (
        <div id={listboxId} role="listbox" className="absolute right-0 top-full z-[100] mt-2 w-32 overflow-hidden rounded-xl border border-black/[0.08] bg-white p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:border-white/[0.08] dark:bg-void-900 dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
          {LIST_WINDOW_OPTIONS.map((option) => (
            <button
              ...
              className={`focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500 flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs font-medium transition-colors ${
                value === option
