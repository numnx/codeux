...
export function ConfirmDialog({ isOpen, options, onConfirm, onCancel }: ConfirmDialogProps) {
  ...
  return (
    <div
      ref={backdropRef}
      className="fixed inset-0 z-[300] flex items-center justify-center bg-void-900/60 backdrop-blur-3xl p-4"
    >
      <div
        ref={(el) => {
          if (el) {
        ...
        className="bg-white/95 dark:bg-void-900/95 w-full max-w-md rounded-3xl shadow-[0_64px_128px_rgba(0,0,0,0.3)] dark:shadow-[0_64px_128px_rgba(0,0,0,0.8)] overflow-hidden border border-black/[0.04] dark:border-white/[0.08] flex flex-col backdrop-blur-2xl"
      >
        <div className="p-8 pb-6">
          <h2 id="confirm-dialog-title" className="text-[1.75rem] font-black text-void-900 dark:text-white font-display tracking-tight leading-none">
            {title}
          </h2>
          <p id="confirm-dialog-body" className="mt-4 text-[13px] font-medium text-void-500 dark:text-void-400 leading-relaxed opacity-80">
            {body}
          </p>
          {destructive && (
            <div className="mt-6 p-4 bg-status-red/5 border border-status-red/10 rounded-2xl flex items-center gap-3">
              <span className="shrink-0 w-2 h-2 rounded-full bg-status-red animate-pulse" />
              <p className="text-[10px] font-black uppercase tracking-widest text-status-red">
        ...
        <div className="flex items-center justify-end gap-3 p-6 bg-black/[0.02] dark:bg-white/[0.02] border-t border-black/[0.04] dark:border-white/[0.04]">
          <button
            ...
            className="px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-void-400 hover:text-slate-900 dark:hover:text-white transition-all active:scale-90"
          >
            {cancelLabel}
          </button>
          {destructive ? (
            <DestructiveConfirmButton
              ...
              label={confirmLabel}
              className="px-6 py-3 text-[10px] font-black uppercase tracking-widest rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-status-red focus-visible:ring-offset-2 active:scale-95 bg-status-red text-white hover:bg-red-600 shadow-[0_8px_24px_rgba(211,47,47,0.3)] transition-all"
            />
          ) : (
            <button
              type="button"
              onClick={() => handleClose(onConfirm)}
              className="px-6 py-3 text-[10px] font-black uppercase tracking-widest rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-500 focus-visible:ring-offset-2 active:scale-95 bg-signal-500 text-void-900 hover:bg-signal-400 shadow-[0_8px_24px_rgba(0,224,160,0.3)] transition-all"
            >
              {confirmLabel}
            </button>
          )}
