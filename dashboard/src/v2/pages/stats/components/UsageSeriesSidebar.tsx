    ...
    <div className="flex flex-col gap-6">
      {Object.entries(groups).map(([grouping, groupSeries]) => (
        <div key={grouping} className="flex flex-col gap-3">
          <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 pl-2">
            {grouping}
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">{s.label}</span>
                  </div>
                  <div className="mt-3 flex items-end justify-between gap-4">
                    <div className="text-lg font-black text-slate-900 dark:text-white">{formatValue(s.id, currentValue)}</div>
                    <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">{getSignalLabel(s.id)}</div>
                  </div>
                </button>
              );
            })}
