import type { ExecutionInvocationRecord, ExecutionInvocationStatus } from "../../../v2/types.js";


// Local helper types for legacy fallback records
export interface LegacyInvocationRecord extends ExecutionInvocationRecord {
  purpose?: string;
  projectRunId?: string;
}
...
export const EMPTY_FILTERS: SystemFilters = {
  status: [],
  purpose: [],
  provider: [],
  errorCategories: [],
...
export function computeLegacyFilteredInvocations(
...
export function computeLegacySummaryMetrics(filteredInvocations: ExecutionInvocationRecord[]): SystemSummaryMetrics {
...
export function computeLegacyAvailablePurposes(allInvocations: ExecutionInvocationRecord[]): string[] {
...
export function computeLegacyAvailableProviders(allInvocations: ExecutionInvocationRecord[]): string[] {
...
export function computeLegacyExternalApiMetrics(allInvocations: ExecutionInvocationRecord[]): ExternalApiMetrics {
  ...
  for (const record of allInvocations) {
    ...
    const purpose = ((record as LegacyInvocationRecord).purpose || "").toLowerCase();
...
export function computeLegacyErrorsByCategory(allInvocations: ExecutionInvocationRecord[]): ErrorsByCategory {
...
export function computeLegacySprintStateSummary(allInvocations: ExecutionInvocationRecord[]): SprintStateSummary {
  ...
  for (const record of allInvocations) {
    const sprintId = record.sprintId || (record as LegacyInvocationRecord).projectRunId || "";
