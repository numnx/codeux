  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4 relative z-10">
        <span className="font-mono text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.14em]">
          {task.id.toUpperCase()}
        </span>
        <PriorityBadge priority={task.priority} />
      </div>

      <h4 className={`text-[15px] font-bold tracking-tight leading-snug mb-5 relative z-10 group-hover:translate-x-0.5 transition-transform duration-300 ${
      ...
      <div className="relative z-10 mb-5 flex items-center gap-2 text-[10px] font-mono font-bold uppercase tracking-[0.14em] text-slate-400">
      ...
      <div className="flex items-center gap-3 mt-auto relative z-10">
        <div className="flex items-center gap-1.5 text-[10px] font-mono font-semibold text-slate-400 dark:text-slate-500">
          ...
          <span className="truncate max-w-[100px]">{task.source}</span>
        </div>
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-mono text-slate-400 dark:text-slate-500">
          <div className="w-5 h-5 rounded flex items-center justify-center bg-black/[0.03] dark:bg-white/[0.03]">
            <span className="text-[9px] font-black font-display text-slate-500 dark:text-slate-400">
              {task.assignee[0]}
            </span>
          </div>
          <span className="font-medium tracking-tight">{task.assignee}</span>
        </div>
      </div>

      <div className="flex items-center justify-between mt-4 pt-4 border-t border-black/[0.04] dark:border-white/[0.04] relative z-10">
        <div className="flex items-center gap-1.5 text-[10px] font-mono text-slate-400 dark:text-slate-500">
          <Clock className="w-3 h-3" strokeWidth={2} />
          <span>{task.time}</span>
        </div>
        <span className="text-[10px] font-mono font-bold text-slate-300 dark:text-slate-700 uppercase tracking-[0.05em]">{timeAgo(task.createdAt)}</span>
      </div>

      {task.dependsOnTaskIds.length > 0 && (
        <div className="relative z-10 mt-3 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400/80">
            ...
            return (
              ...
              >
                <div className="shrink-0">
                  <StatusBadge status={sprint.status} className="!px-2 !py-1 !text-[8px] border-none bg-transparent" />
                </div>
