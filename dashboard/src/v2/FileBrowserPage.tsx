  ...
  if (!selectedProject) {
    return (
      <PageContainer padding="workbench">
        <div class="rounded-[2rem] border border-black/[0.06] bg-white/60 p-8 text-sm text-slate-500 backdrop-blur-md dark:border-white/[0.06] dark:bg-white/[0.04] dark:text-slate-300">
          Select a project first. The file browser launches one containerized snapshot of the active sprint's feature branch.
        </div>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer padding="workbench" className="min-h-full">
      <div class="mb-5 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div class="inline-flex items-center gap-2 rounded-full border border-signal-500/20 bg-signal-500/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-signal-500">
          ...
          <h1 class="mt-3 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">
            Browse and diff the sprint's feature branch.
          </h1>
      ...
      {error && (
        <div class="mb-5 rounded-2xl border border-status-red/20 bg-status-red/10 px-4 py-3 text-sm text-status-red">
          {error}
        </div>
      )}

      {/* Control bar */}
      <div class="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-[1.5rem] border border-black/[0.06] bg-white/70 px-4 py-3 backdrop-blur-md dark:border-white/[0.06] dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center gap-3">
          {statusPill && (
            <span class="inline-flex items-center gap-2 rounded-full border border-black/[0.06] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.14em] dark:border-white/[0.08]">
          ...
          {selectedSession?.featureBranch && (
            <span class="inline-flex items-center gap-1.5 rounded-lg bg-black/[0.03] px-2 py-1 font-mono text-[11px] text-slate-600 dark:bg-white/[0.04] dark:text-slate-300">
              <GitBranch class="h-3 w-3 text-signal-500" strokeWidth={2.2} />
              {selectedSession.featureBranch}
            </span>
          )}
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="flex items-center rounded-xl border border-black/[0.08] bg-black/[0.02] p-0.5 dark:border-white/[0.08] dark:bg-white/[0.03]">
            ...
            >
              ...
              Changes
              {changeCount > 0 && (
                <span class="ml-0.5 rounded-full bg-signal-500/20 px-1.5 text-[10px] font-bold text-signal-600 dark:text-signal-300">
                  {changeCount}
                </span>
              )}
            </button>
          ...
          <button
            ...
            onClick={handleStop}
            disabled={!selectedSession || actionPending}
            class="inline-flex h-9 items-center gap-2 rounded-xl border border-black/[0.08] px-3 text-xs font-semibold text-slate-700 transition hover:border-status-red/30 hover:text-status-red disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/[0.08] dark:text-slate-200"
          >
      ...
      {!runningSession ? (
        ...
        <div class="grid h-[calc(100vh-340px)] min-h-[520px] grid-cols-1 gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
          {/* Sidebar panel */}
          <div class="flex min-h-0 flex-col overflow-hidden rounded-[1.5rem] border border-black/[0.06] bg-white/70 backdrop-blur-md dark:border-white/[0.06] dark:bg-white/[0.03]">
            {mode === "files" ? (
              <>
          ...
          {/* Viewer panel */}
          <div class="flex min-h-0 flex-col overflow-hidden rounded-[1.5rem] border border-black/[0.06] bg-white/70 backdrop-blur-md dark:border-white/[0.06] dark:bg-[#05080d]/80">
            <div class="flex items-center justify-between gap-3 border-b border-black/[0.05] px-4 py-2.5 dark:border-white/[0.06]">
              ...
              {mode === "changes" && (
                <button
                  ...
                  class="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg border border-black/[0.08] px-2.5 text-[11px] font-semibold text-slate-600 transition hover:text-slate-900 dark:border-white/[0.08] dark:text-slate-300 dark:hover:text-white"
...
const LaunchPanel: FunctionComponent<LaunchPanelProps> = ({
  ...
  <div class="relative flex flex-col items-center justify-center overflow-hidden rounded-[2rem] border border-dashed border-signal-500/20 bg-white/55 px-8 py-20 text-center shadow-[0_2px_20px_rgba(0,0,0,0.03)] backdrop-blur-2xl dark:border-signal-500/20 dark:bg-void-800/50">
    <div class="relative z-10 flex max-w-md flex-col items-center gap-5">
      <div class="flex h-16 w-16 items-center justify-center rounded-3xl bg-signal-500/10 text-signal-500 ring-1 ring-inset ring-signal-500/20">
      ...
      {lastError && (
        <div class="w-full rounded-xl border border-status-red/20 bg-status-red/10 px-3 py-2 text-xs text-status-red">
          {lastError}
        </div>
      )}
      ...
      <div class="flex w-full flex-col items-stretch gap-2 sm:flex-row">
        <select
          ...
          disabled={launching}
          class="h-11 flex-1 rounded-2xl border border-black/[0.08] bg-white/80 px-4 text-sm font-medium text-slate-700 outline-none transition focus:border-signal-500/40 disabled:opacity-50 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-slate-200"
        >
        ...
        <button
          ...
          class="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-signal-500 px-6 text-sm font-bold text-void-950 transition hover:bg-signal-400 disabled:cursor-not-allowed disabled:opacity-50"
        >
