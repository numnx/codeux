...
- Docker running (required for default containerized execution).

> **Note:** Code UX starts locally without API keys, but you will configure at least one provider (such as a `JULES_API_KEY` or an authenticated CLI provider) in step 3 before the AI can work.
