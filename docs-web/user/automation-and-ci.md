...
- **`REMOTE`** *(default)* — uses `gh` CLI / GitHub REST API (or GitLab). Reads PR status, comments, CI checks. Required for `WHEN_GREEN` and comment-resolution gates. Code UX refreshes origin before branch-sensitive flows, using bounded fetch timeouts (e.g. `CODE_UX_GIT_FETCH_TIMEOUT_MS`).
- **`LOCAL`** — operates only on local Git state. PR-related features degrade gracefully (no live CI, manual merge prompts only).

`REMOTE` requires `gh auth status` to succeed (or `GITHUB_TOKEN` set). HTTPS GitHub/GitLab tokens are used via temporary Git extraheader where code supports it.

### CLI workflow settings

CLI workflow settings (`Settings -> CLI Workflow`) control behavior like `gitMode` (remote or local), `executionMode` (docker or host), `containerImage`, `containerSetupScriptPath`, branch naming schemes, and generation of worker branches. Local-git projects therefore use the local worker-to-feature merge path consistently during both single-cycle and watch-loop runs. Local-git final merge happens directly in the host repository with no remote host or PR requirement. Remote-git completion uses GitHub/GitLab PR gates and CI checks.
