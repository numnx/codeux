...
> Settings area: `watch-loop`
> Dashboard documentation route: `/docs/settings-watch-loop`

## What This Area Is For

Controls whether live sprint orchestration keeps polling and how frequently it emits work. This page expands the short Settings-page help text into an operator reference for deciding when to change this area, what behavior the controls affect, and what to verify after saving.
...
- **Evaluation Interval:** `watchLoopIntervalSeconds` controls the check frequency. It defaults to `10` seconds and is clamped between `1` and `3600` seconds.
- **Output Interval:** `watchLoopOutputIntervalSeconds` controls the maximum watch-loop runtime before returning an in-progress status and initiating a rerun instruction. It defaults to `300` seconds and is clamped between `60` and `3600` seconds.
