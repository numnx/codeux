...
If CI autofix doesn't trigger, verify the retry cap hasn't been reached and the hosted provider is configured properly. See [Provider Routing](/docs/settings-provider-routing), [MCP Servers](/docs/settings-mcp-servers), and [Configuration and Storage](/docs/settings-configuration-and-storage) for related setup.
