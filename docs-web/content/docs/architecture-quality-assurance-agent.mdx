...
8. if any reviewer requests changes, continue the active provider session with fix instructions when possible, otherwise requeue the task for another implementation pass
...
- before task QA starts, Code UX polls feature PR status with any task-level PR URLs already recorded by the provider. This lets orchestration recover the PR head branch even when the provider PR base branch has drifted from the currently configured sprint feature branch.
...
- startup recovery also clears stale task-coding runtime projections that can otherwise keep sprint QA and merge gates looking active after the real work ended. This includes terminal linked task runs, terminal provider invocations, orphaned provider `task_coding` provider rows, active task-run rows without dispatch/provider/execution linkage, and paused sprint-run rows whose owning sprint is already idle or terminal.
- session sync also clears stale hosted provider task projections when a task records a session id that is missing from the list snapshot and a direct provider lookup returns not found. In that case Code UX fails the stale provider/execution/task-run rows and requeues the task when failed-task retry is enabled.
...
- sprint-scoped task loading falls back to the latest unscoped task run when no task run exists for the active sprint run. This keeps continued provider sessions visible to QA and merge gates after restarts or follow-up messages.
...
- Hosted provider tasks receive a follow-up message on the existing provider session
