...
export const docsRegistry: Record<DocsSlug, DocsRegistryEntry> = {
  ...
  'operations-runbook': {
    ...
    description: "Run these drills against the approved local test project with mocked job/email boundaries only:",
  },
  'operations-security-hardening': {
    ...
    description: "Remote dashboard/API access must use either digest-backed service identities or an OIDC-capable trusted reverse proxy as documented in Secure Headless Server Mode. Never expose credential management by setting only DA...",
  },
  'operations-server-mode': {
