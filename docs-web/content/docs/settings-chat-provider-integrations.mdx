...
The complete setup, identity, verification, retry, session, and limitation matrix is in [External Chat Providers](/docs/architecture-external-chat-providers). Provider details are in [Chat Connector Profiles](./chat-connectors/index.md).
