...
The complete setup, identity, verification, retry, session, and limitation matrix is in [External Chat Providers](/docs/architecture-external-chat-providers). Provider details are in [Chat Connector Profiles](/docs/settings-chat-connectors-overview).
...
See [Security Hardening](/docs/operations-security-hardening) and [Operations Runbook](/docs/operations-runbook) for incident-specific recovery.
