...
Click **Plan sprint**. The planning feedback overlay surfaces both an ETA countdown and an elapsed runtime timer. The planner agent (typically a Gemini, Codex or Claude session — see [Provider routing](/docs/user-providers-and-models)) returns a tree of subtasks with:
