...
The **Memory** page (`/memory`) manages Code UX's two-tier semantic memory system. This system represents what the runtime *learns* automatically from work, separate from the [Knowledge Base](/docs/user-dashboard-knowledge) (reference documents you add) and Persistent Skills (reusable agent instructions).
