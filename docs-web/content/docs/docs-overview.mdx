...
| **An operator** tuning runtime, provider, Git, memory, and security | [Settings →](/docs/settings-overview) |
| **An integrator** building with the MCP server, HTTP API, or realtime protocol | [Developer Reference →](/docs/developer-overview) |
| **An architect** understanding runtime boundaries and control flows | [Architecture →](/docs/architecture-overview) |
| **A dashboard developer** building interfaces and visualizations | [Dashboard Guide →](/docs/user-dashboard-overview) |
| **A contributor** working on the engine itself | [Testing & Quality →](/docs/developer-testing) |
