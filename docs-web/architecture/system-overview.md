...
- Hot realtime, execution projection, provider telemetry, session sync, and dashboard rendering paths must follow the [Code Quality And Performance Contracts](https://github.com/codeux-ai/codeux/blob/dev/docs/architecture/code-quality-performance-contracts.md), including bounded snapshot slices and owner-specific verification commands.
