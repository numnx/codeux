...
describe("SprintsPage", () => {
  ...
  it("renders quota wait clock when present", () => {
    const activeSprint = { id: "sprint-1", number: 1, name: "Wait Sprint", status: "running", showcasePinned: true, createdAt: "2026-06-01T12:00:00Z", updatedAt: "2026-06-01T12:00:00Z", slug: "spr-01", projectId: "proj-1" };
    const quotaMap = new Map();
    quotaMap.set(activeSprint.id, {
      sprintId: activeSprint.id,
      retryAfterIso: new Date(Date.now() + 600000).toISOString(),
      taskCount: 1,
      taskKey: "TASK-1",
      taskTitle: "Wait Task",
      provider: "openai"
    ...
    vi.mocked(useSprintsPageData).mockReturnValue({
      ...
      sortedSprints: [activeSprint],
      showcaseSprints: [activeSprint],
      execution: { status: "not_started" },
      loading: false,
      nextId: "SPR-1",
      sprintKeyPrefix: "SPR",
      planningRoute: { available: true, label: "Test Planning", isDefaultVirtualRoute: false },
      completedCount: 0,
      inWorkCount: 0,
      pendingActionIds: new Set(),
      activeRunsBySprintId: new Map(),
      ...
      overrideSprint: null,
      setOverrideSprint: vi.fn(),
      addTaskForSprint: null,
      setAddTaskForSprint: vi.fn(),
      addTaskSprintTasks: [],
      setAddTaskSprintTasks: vi.fn(),
      virtualProviders: [],
      defaultRouteOptionLabel: "Default",
      defaultModelOptionLabel: "Model",
      ...
      setShowQuicksprint: vi.fn(),
      projectManagementImportProvider: null,
      setProjectManagementImportProvider: vi.fn(),
      canvasImportProvider: null,
      setCanvasImportProvider: vi.fn(),
      quicksprintTemplates: [],
      quicksprintLoading: false,
      feedback: { status: "idle", message: null },
      clearFeedback: vi.fn(),
      clearError: vi.fn(),
    ...
    const { getAllByText } = render(<SprintsPage />);
    expect(getAllByText(/Quota wait/).length).toBeGreaterThan(0);
  });
