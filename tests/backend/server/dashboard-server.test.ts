...
describe("setupDashboardServer", () => {
  it("tries next port when requested port is in use", async () => {
    ...
    const handle = await setupDashboardServer({
      ...
      getStatus: () => ({ ok: true }),
      getExecutionSnapshot: () => ({ projectId: null, projectName: null, sprintRuns: [], taskDispatches: [], recentEvents: [], updatedAt: null }),
      getProjectExecutionSnapshot: () => ({ projectId: null, projectName: null, sprintRuns: [], taskDispatches: [], recentEvents: [], updatedAt: null }),
