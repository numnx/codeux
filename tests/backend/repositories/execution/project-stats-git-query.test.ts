...
describe("queryProjectGitStats", () => {
  it("aggregates git metrics and PR counts accurately over multiple events, handling distinct PR deduplication", async () => {
    ...
    expect(result.totals.insertions).toBe(35); // 10 + 20 + 5
    expect(result.totals.deletions).toBe(6); // 5 + 0 + 1
    expect(result.totals.filesChanged).toBe(4); // 2 + 1 + 1
    ...
    expect(result.taskUsage.get(taskId)?.insertions).toBe(30);
    ...
    expect(result.sprintUsage.get(sprintRunId)?.insertions).toBe(35);
