...
function buildRunCommand(
  ...
  if (availableScript === "preview") {
    ...
    return `HOST=0.0.0.0 PORT="$SPRINT_PREVIEW_PORT" ${runner("preview", previewArgs)}`;
  }

  if (availableScript === "start" || availableScript === "serve") {
    return `HOST=0.0.0.0 PORT="$SPRINT_PREVIEW_PORT" ${runner(availableScript)}`;
  }
