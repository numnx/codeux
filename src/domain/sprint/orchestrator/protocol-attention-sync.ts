...
import {
  buildConflictSummaryMarkdown,
  selectMergedTaskContexts,
  type MergeConflictTaskContext,
} from "./conflict-summary-utils.js";
import type { CycleRunnerArgs } from "./cycle-runner.js";

export function syncProtocolAttentionItems(
  subtasks: Subtask[],
  protocolResult: {
  ...
  deps: {
    projectAttentionService: NonNullable<SprintOrchestratorDependencies["projectAttentionService"]>;
  },
): void {
  ...
  for (const task of protocolResult.awaitingMerge) {
    ...
    deps.projectAttentionService.openItem(buildTaskAttentionPayload({
      projectId,
      sprintId,
      taskId,
    ...
    deps.projectAttentionService.resolveItemsForTask(
      projectId,
      taskId,
