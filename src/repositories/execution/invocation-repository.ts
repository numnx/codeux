...
import {
  requireProject, requireSprint, requireTask, requireSprintRun,
  requireTaskDispatch, requireTaskRun, requireProviderInvocationUsage
} from "./execution-validators.js";
import type { ExecutionRepository } from "../execution-repository.js";
import { parsePayloadJson, asNonEmptyString, stripMarkdown } from "./execution-utils.js";
import {
...
export class InvocationRepository {
  ...
  private getTaskDispatch(id: string) {
    return (this.executionRepository as any).getTaskDispatch(id);
  }

  private getTaskRun(id: string) {
    return (this.executionRepository as any).getTaskRun(id);
  }
  ...
  createExecutionInvocation(input: CreateExecutionInvocationInput): ExecutionInvocationRecord {
    ...
    if (input.sprintRunId) {
      requireSprintRun((id: string) => this.getSprintRun(id), input.sprintRunId);
    }
    if (input.dispatchId) {
      requireTaskDispatch((id: string) => this.getTaskDispatch(id), input.dispatchId);
    }
    if (input.taskRunId) {
      requireTaskRun((id: string) => this.getTaskRun(id), input.taskRunId);
    }
  ...
  createProviderInvocationUsage(input: CreateProviderInvocationUsageInput): ProviderInvocationUsageRecord {
    ...
    const created = requireProviderInvocationUsage((id: string) => this.getProviderInvocationUsage(id), id);
  ...
  updateProviderInvocationUsage(invocationId: string, input: UpdateProviderInvocationUsageInput): ProviderInvocationUsageRecord {
    const current = requireProviderInvocationUsage((id: string) => this.getProviderInvocationUsage(id), invocationId);
    const now = new Date().toISOString();
    this.db.prepare(`
    ...
    const updated = requireProviderInvocationUsage((id: string) => this.getProviderInvocationUsage(id), invocationId);
