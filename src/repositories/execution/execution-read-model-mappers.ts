...
import {
  ...
  assertProviderInvocationPurpose,
  assertProviderInvocationStatus,
  assertTokenUsageSource,
  assertProviderInvocationSource,
  assertExecutionInvocationStatus,
  assertProviderErrorCategory,
  assertInvocationMessageRole,
...
export function mapProviderInvocationUsageRow(row: ProviderInvocationUsageRow): ProviderInvocationUsageRecord {
  return {
    ...
    purpose: assertProviderInvocationPurpose(row.purpose),
    status: assertProviderInvocationStatus(row.status),
    model: row.model,
    executionMode: row.execution_mode as ProviderInvocationUsageRecord["executionMode"],
    nativeSessionId: row.native_session_id,
    usageSource: assertTokenUsageSource(row.usage_source),
    ...
    invocationSource: assertProviderInvocationSource(row.invocation_source),
...
export function mapExecutionInvocationRow(row: ExecutionInvocationRow): ExecutionInvocationRecord {
  return {
    ...
    type: row.type,
    status: assertExecutionInvocationStatus(row.status),
    provider: row.provider,
    model: row.model,
    systemPrompt: row.system_prompt,
    errorMessage: row.error_message,
    lastErrorCategory: assertProviderErrorCategory(row.last_error_category),
...
export function mapExecutionInvocationMessageRow(row: ExecutionInvocationMessageRow): ExecutionInvocationMessageRecord {
  return {
    ...
    role: assertInvocationMessageRole(row.role),
