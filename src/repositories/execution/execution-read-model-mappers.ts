...
export function mapProviderInvocationUsageRow(row: ProviderInvocationUsageRow): ProviderInvocationUsageRecord {
  return {
    ...
    rawUsageJson: row.raw_usage_json ? (parsePayloadJson(row.raw_usage_json) as Record<string, unknown>) : null,
  };
}
...
export function mapExecutionInvocationMessageRow(row: ExecutionInvocationMessageRow): ExecutionInvocationMessageRecord {
  return {
    ...
    toolCallsJson: row.tool_calls_json ? (parsePayloadJson(row.tool_calls_json) as Record<string, unknown>) : null,
    metadata: row.metadata_json ? (parsePayloadJson(row.metadata_json) as Record<string, unknown>) : null,
    createdAt: row.created_at,
  };
}
