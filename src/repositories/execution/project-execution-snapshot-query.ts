  ...
  projectId: string,
    ...
    return {
      ...
      sprintRuns: sprintRuns.map((row) => mapExecutionSprintRunSummaryRow(
        ...
        repository.withWallTime(usageBySprintRunId.get(row.id), wallTimeBySprintRunId.get(row.id) || 0),
      )),
      taskDispatches: taskDispatches.map((row) => mapExecutionTaskDispatchSummaryRow(
        row,
        repository.withWallTime(usageByTaskId.get(row.task_id), wallTimeByTaskId.get(row.task_id) || 0),
      )),
      connections: [],
