...
import { loadExecutionSnapshotData } from "./execution/execution-snapshot-loader.js";
import { accumulateUsageStats, buildChartSeries, createEmptyUsageTotals } from "./execution/execution-stats-aggregator.js";
import type {
...
export class ExecutionRepository {
  ...
  getProjectExecutionSnapshot(projectId: string): ExecutionDashboardSnapshot {
    ...
    const { sprintRuns, taskDispatches, taskRunByDispatchId, runtimeEvents } = loadExecutionSnapshotData(
      this.db,
      this.storage,
      projectId,
      this.compareExecutionTaskDispatchSummaryRows.bind(this),
      this.compareExecutionRuntimeEventSummaryRows.bind(this)
    );
  ...
  getProjectStatsSnapshot(
    ...
    const {
      ...
      this.mergeUsageTotals.bind(this),
      this.mergeUsageMap.bind(this),
    ...
    const chartSeries = buildChartSeries(
      ...
      buckets as any,
      normalized.bucketSizeMs,
      Array.from(providerUsage.keys()),
      Array.from(purposeUsage.keys())
    );
    ...
    return {
      ...
      tokenSources: Array.from((tokenSourceCounts as Map<string, number>).entries())
