...
import {
  getWallTimeTotalsWithCache,
  buildWallTimeCacheKey,
  WallTimeCacheEntry,
  getWallTimeTotalsByTaskIdsForRange,
  getWallTimeTotalsBySprintRunIdsForRange,
} from "./execution/execution-wall-time-query.js";
...
export class ExecutionRepository {
  ...
  private readonly taskWallTimeCache = new Map<string, WallTimeCacheEntry>();
  private readonly sprintRunWallTimeCache = new Map<string, WallTimeCacheEntry>();
  ...
  createTaskRun(input: CreateTaskRunInput): TaskRunRecord {
    try {
      ...
      if (created.taskId) {
        this.taskWallTimeCache.delete(buildWallTimeCacheKey(created.projectId, created.taskId));
      }
      if (created.sprintRunId) {
        this.sprintRunWallTimeCache.delete(buildWallTimeCacheKey(created.projectId, created.sprintRunId));
      }
  ...
  updateTaskRun(taskRunId: string, input: UpdateTaskRunInput): TaskRunRecord {
    try {
      ...
      if (updated.taskId) {
        this.taskWallTimeCache.delete(buildWallTimeCacheKey(updated.projectId, updated.taskId));
      }
      if (updated.sprintRunId) {
        this.sprintRunWallTimeCache.delete(buildWallTimeCacheKey(updated.projectId, updated.sprintRunId));
      }
      this.notifyRealtime(updated.projectId, false);
      return updated;
      } catch (error) {
  ...
  appendTaskRunEvent(
    ...
    if (taskRun.taskId) {
      this.taskWallTimeCache.delete(buildWallTimeCacheKey(taskRun.projectId, taskRun.taskId));
    }
    if (taskRun.sprintRunId) {
      this.sprintRunWallTimeCache.delete(buildWallTimeCacheKey(taskRun.projectId, taskRun.sprintRunId));
    }
  ...
  private getWallTimeTotalsByTaskIds(projectId: string, taskIds: string[], nowIso: string): Map<string, number> {
    return getWallTimeTotalsWithCache(this.storage, projectId, taskIds, this.taskWallTimeCache, "task_id", nowIso);
  }

  private getWallTimeTotalsBySprintRunIds(projectId: string, sprintRunIds: string[], nowIso: string): Map<string, number> {
    return getWallTimeTotalsWithCache(
      this.storage,
      projectId,
      sprintRunIds,
      this.sprintRunWallTimeCache,
      "sprint_run_id",
      nowIso,
    );
  }

  private getWallTimeTotalsByTaskIdsForRange(
    projectId: string,
    rangeStartIso: string,
    rangeEndIso: string,
    nowIso: string,
  ): Map<string, number> {
    return getWallTimeTotalsByTaskIdsForRange(this.db, projectId, rangeStartIso, rangeEndIso, nowIso);
  }

  private getWallTimeTotalsBySprintRunIdsForRange(
    projectId: string,
    rangeStartIso: string,
    rangeEndIso: string,
    nowIso: string,
  ): Map<string, number> {
    return getWallTimeTotalsBySprintRunIdsForRange(this.db, projectId, rangeStartIso, rangeEndIso, nowIso);
  }
