...
export class ExecutionRepository {
  ...
  withWallTime(usage: ExecutionUsageTotals | undefined, wallTimeMs: number): ExecutionUsageTotals {
  ...
  getWallTimeTotalsByTaskIds(taskIds: string[], nowIso: string): Map<string, number> {
    if (taskIds.length === 0) {
      return new Map();
    }
  ...
  getWallTimeTotalsBySprintRunIds(sprintRunIds: string[], nowIso: string): Map<string, number> {
    if (sprintRunIds.length === 0) {
      return new Map();
    }
  ...
  listActiveAttentionRowsForProject(projectId: string): ProjectAttentionSummaryRow[] {
