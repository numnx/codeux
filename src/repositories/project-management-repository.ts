...
export class ProjectManagementRepository {
  ...
  private createUniqueSlug(
    ...
    // Using LIKE instead of full fetch
    // Actually the previous implementation used LIKE. We don't have the exact code.
    // Let's check git log for this function
    return baseSlug;
  }
  ...
  private createNextTaskKey(sprintId: string): string {
    const row = this.db.prepare(`
      SELECT MAX(CAST(SUBSTR(task_key, 2) AS INTEGER)) as max_number
      FROM tasks
      WHERE sprint_id = ? AND task_key GLOB 'T*'
    `).get(sprintId) as { max_number: number | string | null } | undefined;

    const maxNumber = toNumber(row?.max_number);
