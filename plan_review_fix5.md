...
const regex = /const arrayBuffer = await response\.arrayBuffer\(\);\s*if \(arrayBuffer\.byteLength > 5 \* 1024 \* 1024\) \{\s*throw new Error\("Response body exceeds maximum allowed size for proxied preview"\);\s*\}\s*const bodyBuffer = Buffer\.from\(arrayBuffer\);/;
...
7. Use the `run_in_bash_session` tool to run tests via `pnpm run test:backend -- tests/backend/services/sprint-preview-service-unit.test.ts tests/backend/server/preview-host-utils.test.ts tests/backend/services/sprint-preview-docker-plan.test.ts` and `pnpm run typecheck` to ensure functionality is fully working.
