...
- During Electron shutdown, the `before-quit` event cancels the first quit attempt and yields to the asynchronous `stopServer()` process. Once the embedded `code-ux-server` is cleanly closed, the process explicitly exits using `app.exit(exitCode)` to prevent stale recursive app quit behaviors, especially on macOS.
...
- The desktop shell renders only the resolved dashboard origin and same-port sprint preview origins that match `preview-([a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\.localhost` internally. Other `http:`, `https:`, and `mailto:` navigations are denied in the renderer and opened through the host operating system after scheme validation; all other schemes are blocked.
