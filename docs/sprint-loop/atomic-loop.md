...
- Worker-generated clarification replies normalize noisy CLI output before posting back to Jules; provider bootstrap logs and `{ "response": "..." }` execution envelopes are stripped so only the answer body is sent.
