...
- If a main-branch merge conflict or blocker occurs, and there is an active worker attention item open (e.g. `ownerType: "worker"`), the watch loop will not immediately pause and exit. Instead, it will transition to a wait state to allow the worker time to pick up the job and auto-resolve the conflict. The sprint will only transition to a paused state (requiring manual intervention) once the attention item is escalated to a human (e.g., `ownerType: "human"`).
