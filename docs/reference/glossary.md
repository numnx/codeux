...
## Memory
Runtime-learned short-term sprint evidence and long-term project claims, embedded and injected into prompts according to agent memory config. Distinct from Knowledge.

## Knowledge
Project-scoped reference library of documents, ingested/uploaded/imported separately from Memory, embedded locally, and attached to agent presets via subscriptions. Agents use `search_knowledge` for exact passages.
...
## Route / Routes
Server endpoint or Dashboard view handler invoked by requests, with execution telemetry and tracking logged by route invocation IDs (e.g., `task_coding`).
