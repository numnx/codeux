...
- The legacy project metadata `defaultBranch` column is retained for project records created before the scoped settings model and for display/initialization context, but sprint orchestration, branch preflight, and final merge targets use the resolved scoped settings (`git.defaultBranch` fallback is resolved based on scoped overrides too). A project inheriting a system default of `dev` must merge sprint completion PRs into `dev`, even if the older project row still says `main`.
