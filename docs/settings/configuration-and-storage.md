...
- Packaged Windows Electron uses a `low-power` desktop render profile exposed by the preload bridge. The dashboard clears custom background image/pattern styling and renders a static background in this profile, while the BrowserWindow itself is opaque instead of transparent.
