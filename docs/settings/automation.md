...
> Settings area: `automation`
> Dashboard documentation route: `/docs/settings-automation`

## What This Area Is For

Controls how much Code UX may continue without pausing for operator decisions. This page expands the short Settings-page help text into an operator reference for deciding when to change this area, what behavior the controls affect, and what to verify after saving.
...
When concurrency is allowed, multiple tasks may run simultaneously in separate worktrees or containers. A shared host repository may experience temporary dirty states during orchestrator synchronization or concurrent operations that touch the shared Git index. High automation and high concurrency require robust CI validation and clean merge isolation.
...
If the orchestrator stops unexpectedly with dirty-workspace complaints, ensure there isn't a stale uncommitted file blocking the next dispatch, and consider reducing concurrency if host-index contention is suspected.
