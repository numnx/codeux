...
*   **Quota Waits:** Gallery Sprint cells surface provider quota waits using `deriveSprintQuotaWaits` and `findActiveQuotaWait` to render a quota reset clock when any task in that sprint has an active `cli_provider_quota_wait` event. The clock uses `WaveFluid` and `BorderTrace` for motion, but must always provide static visible text, semantic status copy, keyboard-reachable controls, and non-motion fallback cues for accessibility and reduced motion. The cell retains task links and actions while the provider waits to continue.
