...
- **Keyboard Contracts**: Shell dropdowns (like the project or sprint selectors using `useDropdownKeyboard`) must support `[role="option"]` navigation to not select disabled items or inputs, and focus restoration to their trigger when closed.
