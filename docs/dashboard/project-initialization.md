...
Directory browsing must keep keyboard focus on the active control. Loading, current path, empty directory lists, failed loads, and selected-path confirmation are visible in the picker or directly below the associated field and are also exposed through polite live regions.

The frontend modal populates the backend payload with `name`, `sourceType` (local, git), `sourceRef` (existing path or new repo slug/URL), `cloneDir` (optional target directory for clone or local setup), `setup` (optional scope configuration with `enabled` boolean and artifact selections), and `initMode` (`existing`, `new-local`, `new-remote`). Local project and new local project directory paths remain optional; when missing, the backend resolves the missing directory using the home directory during initial root validation (`resolveNewLocalProjectDir` and `validateSafeClonePath`). `defaultBranch` is passed optionally and resolved, or falls back to 'main'.

During async project creation, close/cancel/submit controls expose the busy reason and suppress duplicate activation. Submission failures keep the modal open, show retryable feedback, and resubmit the same form payload when the operator chooses Retry. Setup operations are idempotent; partial failures like a single docs embedding error will be reported in diagnostics without failing the entire setup, and artifacts inside `.code-ux/` are generated safely.
