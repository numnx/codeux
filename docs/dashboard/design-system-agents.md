...
- Chat runtime work tools are part of the same `AgentAvatarScene` lifecycle. They are rebuilt independently from avatar config changes, disposed on tool swap/unmount, and should use premium readable geometry with warm metal/rubber surfaces, signal accent inlays, a subtle halo, and a local point light. Do not shrink tools into icon-like props; they must remain legible beside the avatar in the default `/chat` canvas and under `/chat?stageTool=<kind>` design review.
