...
Internal MCP clients use the same importer services through `manage_sprints` action `import_issues`. Code UX explicitly integrates configured adapters for imports (such as GitHub/GitLab explicit numbers and Jira searches) rather than merely assuming support based on enum definitions. For Jira and other external integrations, search-based imports and explicit-key imports both attach the selected issue descriptions to the planner scope. The `includeConversation` option controls whether comments are attached to the imported description. The lifecycle of these imported, linked issues is managed explicitly via `PUT /api/sprints/:sprintId/linked-issues`. For payload examples covering search-only imports, assigned-work searches, explicit Jira keys, explicit GitHub/GitLab issue numbers, explicit external IDs, canvas identifiers, sprint attachment, and plan-after-import flows, see [MCP Tools and Contracts: `manage_sprints import_issues`](../mcp/tools-and-contracts.md#manage_sprints-import_issues).
