...
3. **Shell Navigation:** Top-nav project/sprint/techstack selectors, notification menus, theme/Docker controls, sidebar links, and the bottom `KineticDock` must keep their accessible names when the visual treatment becomes icon-only. Tooltip text may be visual-only, but the control itself needs the stable name.
   The `KineticDock` cannot use continuous animation as the only active-route cue. Each route link keeps `aria-current="page"` and `data-active="true"` when selected, plus a static active background and dot that remain visible with reduced motion. Nested routes should resolve to their parent destination so mobile dock state stays obvious during route transitions. Top-nav project selectors persist the active project and use bounded scrollable listboxes so long lists stay inside the header overlay. Mobile menus gracefully collapse items such as the project and sprint selectors to avoid horizontal overflow on narrow widths.
