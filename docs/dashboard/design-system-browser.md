...
- **Accessibility**: Browser panes must maintain focus management, apply `aria-busy` states on rails and actions during async operations, use polite live-region announcements for state changes, and maintain stale cached content visibly during foreground loads or errors instead of blanking the screen.
