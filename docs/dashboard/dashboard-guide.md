...
- Sprint and Quicksprint planning route controls list virtual provider instances by their settings-page names (for example, `Codex Primary`) with provider brand icons, keep connected worker routes visually distinct, show resolved default route/model labels such as `Default Route (Codex Primary)` and `Default Model (gpt-5.5)`, and scope model override choices to the selected provider instance so stale overrides clear when the route changes.
...
- Planning and prompt-improvement requests continue server-side if the browser tab is refreshed or closed. The overlay's `Cancel Active Request` action sends an explicit cancellation request, while `New Sprint` and `New Quicksprint` detach the current planning run from the visible composer, immediately reset the form controls, and leave the old run to finish without closing or mutating the fresh composer.
