...
- **Task Card Affordances**: Kanban task cards explicitly show a `cursor-grab` (`cursor-grabbing` while dragging) pointer affordance to differentiate them from static elements. Optimistic state transitions rely on `aria-disabled` styling rather than raw opacity, providing an accessible structure for screen readers while preventing pointer events.
- **Dependency Indicators**: Task dependency chips augment standard status colors with `lucide-preact` icons (CheckCircle2, PlayCircle, Clock, AlertCircle) to maintain accessibility for color-blind users and avoid relying on color alone.
