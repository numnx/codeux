...
- The Live sidebar now renders `Attention Queue` as its own standalone card under the shared execution timeline context, with the same card chrome, optional collapsible header, and claim/resolve/dismiss controls while keeping the execution runtime card focused on runs, dispatch, connections, and timeline data
