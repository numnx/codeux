...
- When `waitForCiBeforeFeatureMerge` and `waitForJulesCiAutofix` are both enabled (REMOTE mode), completed tasks are held in work state while feature-PR checks are pending or failed.
