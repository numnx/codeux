...
- **App-Shell Navigation**: Primary navigation must keep keyboard and assistive-tech state aligned with the visual shell. Closed mobile sidebars are hidden with `aria-hidden` and `inert`, open mobile sidebars trap focus, Escape closes them and restores focus to the menu trigger, and route changes close the mobile sidebar. Sidebar and dock links use stable accessible names with `aria-current="page"` for the active route. Project and sprint selector triggers expose selected, busy, expanded, controlled, and disabled states; listbox options expose `aria-selected`.
