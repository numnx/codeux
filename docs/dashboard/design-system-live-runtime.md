...
- Cancel, force-cancel, and force-complete runtime controls are destructive and must request confirmation before invoking backend actions. Confirmation copy should name the target sprint run, dispatch, or task using the user-facing sprint name, task key, or task title rather than an opaque runtime ID when that label is available.
- The action dispatcher must guard pending runtime actions with an immediate in-memory pending set, not only rendered disabled state, so rapid repeat activation cannot submit duplicate pause, cancel, retry, rerun, claim, resolve, dismiss, reset-usage-limit, or force-cancel requests before the next render.
