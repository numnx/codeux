...
- **Assigned Worker Summaries**: Assigned worker summaries (primary and overflow arrays) must be strictly equivalent across snapshots to reuse references. This checks equivalence across assignment ID, endpoint identity, display name, connection properties, transport, role, statuses, and capabilities. Shallow checks that only verify lengths and endpoint IDs are insufficient, as status or capability shifts will otherwise fail to trigger a snapshot update.
