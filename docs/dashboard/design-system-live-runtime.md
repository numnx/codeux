...
- Runtime action buttons use `aria-disabled` for pending states when focus should remain stable, suppress activation while pending or unavailable, and expose the current reason through the visible label, `title`, or screen-reader status text.
