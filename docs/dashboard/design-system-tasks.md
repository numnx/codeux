...
## 6. Task Board Interaction States and Dependency Feedback
*   **Filtering and Reordering:** Use GSAP stagger animations (e.g. `useLayoutEffect` with `gsap.fromTo`) when task board filters change or task data live-updates to prevent jarring layout shifts. Ensure cards appear with smooth opacity and vertical translation (`y: 15` to `0`).
*   **Action Affordances:** Inline action buttons (Rerun, Inspect, Dependencies, External Link) should be consistently placed in the top-right absolute container, revealed on hover or keyboard focus (`focus-within`), and rely on consistent `lucide-preact` icons. If an action is disabled, visually indicate it (`disabled:opacity-50 disabled:cursor-not-allowed`).
*   **Dependency Indicators:** Must use explicit hover `title` explanations and keyboard focus visibility (`focus-visible:ring-signal-500/30`) rather than relying on color alone.
