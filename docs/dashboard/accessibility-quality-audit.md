...
- Dialogs, alert dialogs, popovers that act as dialogs, Add Project/Add Task modals, destructive confirmations, and unsaved-change prompts must trap focus while open (via `useFocusTrap` hook and `<Overlay>`) and restore focus after close. If the trigger disappears, focus falls back to a sensible page landmark. Nested dialogs can utilize `suspendFocusTrap` to temporarily yield focus control.
...
- Empty, low-data, stale-data, success, pending, and background-refresh states stay polite. They should preserve existing content where possible instead of replacing the whole workbench during refresh. Empty states must present actionable recovery logic or an `EmptyState` component coupled with an `aria-live` announcement, rather than silently rendering an empty parent DOM element.
