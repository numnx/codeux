...
The `submitMode` controls whether the scheduled or immediate request executes as `"plan_only"` (leaving the sprint ready for review) or `"plan_and_start"` (orchestrating execution immediately after planning finishes).
...
- `execute`: Plans a quicksprint. Payload supports `taskCount`, `noTaskLimit`, `submitMode`, `routeOverride`, and `modelOverride`. The `submitMode` defaults to `"plan_only"`.
- `start`: Alias for execution where `submitMode` defaults to `"plan_and_start"`.
