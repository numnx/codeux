...
```bash
# /health checks liveness (listener is up)
curl --fail http://127.0.0.1:4445/health

# /ready checks readiness (startup recovery finished, dependencies available)
curl --fail http://127.0.0.1:4445/ready
```

Note: `/health` may pass before `/ready` passes. Startup readiness refuses execution until background recovery, branch reaping, and key providers are operational. If credentials are bad, `/ready` returns 503, but `/health` is UP.
...
| Symptom | Safe Diagnostics & Likely cause | Recovery & Fix | Rollback & Escalation |
| --- | --- | --- | --- |
| Startup fails with a server-mode token error | `--server-mode` or `CODE_UX_SERVER_MODE=true` is set without an explicit valid bearer token. | Set `MCP_HTTP_AUTH_TOKEN` or `MCP_HTTPS_AUTH_TOKEN`, or pass the matching CLI flag. Use at least 32 bearer-safe characters. | Do not use weak tokens. Revert configuration if headless is not needed. |
| Startup fails when binding `0.0.0.0`, `::`, or a LAN address | MCP HTTP is reachable beyond loopback without an active token. | Configure an explicit bearer token and put TLS/auth network controls in front of the HTTP listener. | Revert bind to `127.0.0.1`. |
| Dashboard URL is unavailable | Expected in server mode. | Use MCP HTTP clients and `/health` or `/ready`. Start a separate dashboard-mode process only when an operator UI is required. | No rollback needed. |
| HTTP returns `401 Unauthorized` | Missing `Authorization: Bearer <token>`, wrong token, duplicate authorization headers, or a client still using the old token after rotation. | Reinstall or update the client secret, reconnect. | Avoid printing headers in diagnostics. Escalate if valid tokens fail. |
| HTTP returns `400` on a new MCP session | The first request was not JSON-RPC `initialize`, or `mcp-session-id` / `x-code-ux-agent` was malformed. | Let the MCP SDK initialize the session, or clear stale session state and reconnect. | Revert client updates if incompatible. |
| Session cap errors appear | Too many active Streamable HTTP sessions, usually from leaked clients or a cluster larger than the default cap. | Stop stale clients, shorten `MCP_HTTP_SESSION_TIMEOUT_MS`, or raise `MCP_HTTP_MAX_SESSIONS` within server capacity. | Revert cap increases if memory pressure occurs. |
| Worker appears stale or offline | Heartbeats stopped, the worker process is down, network access failed, or the stable connection key changed unexpectedly. | Restart the worker with the same `--connection-key`, verify `/ready`. | Escalate if worker repeatedly drops. Check logs for bounded connection metadata. |
...
| `/health` passes but `/ready` fails | Listener is alive but runtime readiness has not completed or the server is degraded. | Wait for startup recovery to finish, then inspect structured logs. | Use `/ready` for load balancer readiness gates. Escalate if startup recovery loops. |
...
| Secret values appear in an exported settings bundle | The export was explicitly approved with `includeSecrets: true`. | Store the bundle only in approved secret storage, rotate exposed credentials if it was shared. | Prefer redacted exports for review. |
## Related Docs
