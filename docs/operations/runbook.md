...
- Packaged Windows Electron runs use an opaque desktop window and a low-power dashboard render profile. This disables custom full-window image, pattern, and animated backgrounds inside the desktop shell to avoid Chromium tile-memory exhaustion (`tile_manager.cc:1012 WARNING: tile memory limits exceeded`). Browser users and non-Windows desktop builds keep the normal visual background settings.
