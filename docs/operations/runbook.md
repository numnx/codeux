...
- Startup logs will show a structured result detailing counts of pruned elements, failed vacuums (`vacuumFailed`), and WAL checkpoint failures (`checkpointFailures`). WAL checkpoint failures are non-fatal, busy checkpoints are safe to retry later.
...
5. Confirm `/health` and `/ready` probes:
   - `/health`: Liveness probe. Reports `UP` or `DOWN` based on `settingsDb`, `dashboardBind`, and `mcpService` components. In dashboard mode it is served by the dashboard server; in server mode it is served by the MCP HTTP listener.
   - `/ready`: Readiness probe. Reports `READY` or `NOT_READY` for the overall status, along with `UP`/`DOWN` for individual components (`settingsDb`, `dashboardBind`, and `mcpService`). A success means the server considers required startup/runtime dependencies ready enough to serve normal traffic. It does not validate every provider, project, Docker workspace, or external service.
...
- During shutdown (e.g., Electron quit or server shutdown signals), `ShutdownContainerService` gracefully kills active `code-ux.*` containers by first requesting active dispatch stops and then issuing SIGINT/SIGTERM/SIGHUP to remaining containers, while preserving workspace volumes. Code UX disposes the command-spawner host before Docker cleanup (`DockerRuntimePruneService` and `DockerAssetPruneService`). `DockerRuntimePruneService` safely prunes stale per-runtime paths and shared temp paths after their age threshold while preserving active roots/Codex homes. `DockerAssetPruneService` cleans up orphaned workspace volumes, login containers, helper containers, and temporary credential directories on startup. Do not instruct operators to run broad manual `docker system prune` commands.
...
- Startup recovery is orchestrated centrally by `RuntimeStartupRecoveryService`. It handles reconciliation for interrupted CLI sessions, local/provider dispatches, retry waits, QA review runs, orphaned provider invocations, terminal dispatches, interrupted task runs, stale paused sprints, and recoverable sprint runs. It also moves premature merge-conflict escalations back to automatic handling (if guardrails allow) and cancels duplicate active task dispatches for the same task/sprint run. Recovered-state logging surfaces these distinct outcomes to the dashboard.
...
- On startup, active `queued` and `running` sprint runs are resumed according to the `restartSprintPolicy`: `continue` resumes automatically in place (default); `pause` or `cancel` active runs before recovery resumes anything.
- The `restartInvocationPolicy` controls active/interrupted provider rows when sprint continuation is enabled: `continue` preserves live labelled containers, `restart` stops labelled containers without removing preserved volumes and requeues task-backed work, and `cancel` stops labelled containers without automatic retry.
