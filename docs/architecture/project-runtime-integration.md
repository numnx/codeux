...
- Duplicate Jules clarification auto-replies no longer keep a task hidden as indefinitely running. The first duplicate within the configured cooldown is still suppressed, but an unchanged `AWAITING_USER_FEEDBACK` state after the cooldown remains `BLOCKED` and requires manual review.
