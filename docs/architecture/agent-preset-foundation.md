...
Skill markdown is kept outside project worktrees. Each storage is materialized as an internal local-only Git repository, and injected into the container read-only at `/code-ux/persistent-skills/<storage-id>/` during provider execution. Agents are expected to read `SKILL.md` files from this container-safe mount or use `search_skills` to retrieve knowledge, ensuring persistent skills do not become uncommitted project files or depend on host-path capabilities. Every skill mutation through `manage_skills` synchronizes and commits the isolated repository, yielding an immutable revision for runtime provenance. Git commands use the shared `alpine/git` helper-container boundary, so this feature never requires host Git. SQLite remains the query projection during the migration: frontmatter fields (`title`, `description`, `tags`, `appliesTo`, `version`) and the markdown body are indexed in `skills`, while `skill_embeddings` stores model id, dimension, chunk index, content hash, and vector blob.
...
- optional persistent skill storage attachments (default-off, isolated from project workspaces via a containerized Git repository, and injected only for enabled attached agents at provider runtime as read-only snapshots at `/code-ux/persistent-skills/<storage-id>/`, never exposing arbitrary host paths). Agents must use `manage_skills` to create or update durable skills safely without arbitrary workspace writes, and retrieve them via `search_skills`.
