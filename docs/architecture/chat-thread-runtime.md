...
When triggered on a virtual chat route, the system runs a dedicated execution invocation against the selected virtual chat worker (via `ProviderTextInvocationService`) and asks it to produce a compacted markdown handoff of the full thread history.
