...
- Refactor the connection lifecycle so Code UX can instantiate either the SQLite or Postgres adapter based on `AppConfig` or environment variables.
...
- The Postgres migration will be opt-in via configuration (`CODE_UX_DB_URL=postgres://...`).
...
- **Rollback**: To revert to SQLite, users unset the `CODE_UX_DB_URL` environment variable. The system will start using the local SQLite file.
