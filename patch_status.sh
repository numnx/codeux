...
+              <span className="sr-only">Status: </span>
             ...
             <span className="inline-flex items-center gap-1.5 rounded-full border border-signal-500/25 bg-signal-500/10 px-3 py-1.5 text-[11px] font-bold text-signal-700 dark:text-signal-300">
+              <span className="sr-only">Status: </span>
               <Loader2 className="h-3 w-3 animate-spin" strokeWidth={2.2} /> {sprint.status === "running" ? "Stopping" : "Starting"}
             </span>
...
@@ -365,6 +370,7 @@
         <div className="flex min-w-0 flex-1 flex-col items-end justify-center gap-3 pr-2 lg:flex-row lg:items-center">
           <SprintControls
             isActive={sprint.status === "running"}
