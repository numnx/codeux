...
const enMatch = code.match(/en:\s*(\{[\s\S]*?\})\s*,\s*de:/);
if (!enMatch) {
    console.error("Could not find en block");
    process.exit(1);
}

// Convert the en block string to a JS object
let enObjStr = enMatch[1];
// We need to parse this string to JSON. But it's not strictly JSON, it's JS literal.
// We can use new Function to evaluate it.
let en;
try {
    en = new Function(`return ${enObjStr}`)();
} catch(e) {
    console.error("Failed to parse en block", e);
    process.exit(1);
}

const deMatch = code.match(/de:\s*(\{[\s\S]*?\})\s*,\s*es:/);
let deObjStr = deMatch[1];
let de = new Function(`return ${deObjStr}`)();
...
function objectToString(obj) {
    ...
    for (const [k, v] of Object.entries(obj)) {
        if (typeof v === 'string') {
            ...
            lines.push(`    ${k}: { one: ${JSON.stringify(v.one)}, other: ${JSON.stringify(v.other)} }`);
        }
    }
    return '{\n' + lines.join(',\n') + '\n  }';
}

const enStr = objectToString(en);
const deStr = objectToString(de);
const esStr = objectToString(es);
