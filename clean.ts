...
describe("ProjectManagementRepository", () => {
  ...
  it("handles originalPrompt in sprints and supports clearing tasks", async () => {
    ...
    expect(sprint.sprintKey).toBeNull();
    ...
    expect(updated.sprintKey).toBe("NEW-KEY");

    const updatedAgain = repository.updateSprint(sprint.id, {
      sprintKey: null,
    });
    expect(updatedAgain.sprintKey).toBeNull();

    expect(repository.getSprint(sprint.id)?.sprintKey).toBeNull();
    expect(repository.findSprintByProjectAndNumber(project.id, sprint.number!)?.sprintKey).toBeNull();
    expect(repository.findSprintByProjectAndNumber(project.id, 999)).toBeNull();
    });
});
